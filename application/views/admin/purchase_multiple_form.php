...
<?=form_close()?>
 ...
 <table class="table table-bordered table-hover tablesorter center"> 
       ...
       <tbody id="SourceWrapper">
         ...
         <tr class="clonethis">
         <td class="no">1</td>
              <td><?=form_input('product_code[]','','class="form-control-discount pcode autocomplete"  onkeyup="fillWithPurchase(this.value,event)" autocomplete="off"');?> <div class="display"></div></td>
             <td> <?=form_input('product_name[]','','class="form-control-discount" autocomplete="off"');?></td>
              ...
              <td> <?=form_dropdown('unit[]',array("pcs"=>"pcs"),'',' class="form-control-discount clonelastone" onkeyup="admincloneform(event)"  onchange="grabprice(this.value,event)" autocomplete="off"');?> <?=form_hidden('item_count_hidden[]','');?><?=form_hidden("category[]","")?>
</td>
              <!--  <td><?php //form_input('item_count[]',0,' class="form-control-discount clonelastone" autocomplete="off"');?>
                
               </td> -->

              <td width="100"> <?=form_input('price[]','',' class="form-control-discount price" onkeyup="admincloneform(event)" autocomplete="off"');?></td>
...
<td colspan="4" align="right">
 <?php echo form_button('save','SAVE','class="btn btn-primary btn-lgy" onclick="purchase_insert(\'purchase_multiple\')"');?>
